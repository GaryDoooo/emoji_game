...
from account_manage import account, print_account_list, load_account
...
def show_room():
    ...
    a=os.popen('find vending_accounts/ -name "*.p" |  grep -oP "(?<=vending_accounts/)\w+"').read().split("\n")[:-1]
    for user in a:
        user_account=load_account(user)
        print_inventory(emoji,user_account.emoji_you_have,title=user+"'s emoji")
...
def main():
    ...
    while True:
        ...
        if action=="v" or action=="V":
            user.points,user.emoji_you_have=vending(user.points,price,emoji,user.emoji_you_have)
