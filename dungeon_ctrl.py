...
class dungeon_screen:
    def __init__(self,dungeon_name="==== Dungeon ====",subtitle="Use Arrow keys to move your butt. ENTER to interact."):
    ...
    # self.title_win = curses.newwin(1, width, 1, 1)
        ...
        print(term_io.delete.whole_screen + term_io.cursor.upper_left
                + term_io.bgcolor.reset + term_io.fgcolor.Yellow + self.dungeon_name)
    ...
    def close(self):
        ...
        # time.sleep(1)
        # curses.endwin()
        print(term_io.bgcolor.reset+term_io.fgcolor.default+term_io.delete.whole_screen)
...
def menu_action(screen,item):
    ...
    screen.add_log(rand_pick(
        item.narratives[item.actions[action]]
            ))
    return item.actions[action]

def take_action(screen, user,next_block):
    if next_block == 1100:
...
def rand_mine(screen,user,rate=40):
    if randint(1,rate)==1:
        ...
        screen.add_log(term_io.fgcolor.green +
                        "You found %d points on the floor and picked them up." % points
...
def portal_entry(user):
    ...
    while True:
        ...
        if key == 'enter':
            user = take_action(screen,user,next_block)
        else: 
