...
def print_menu(menu_item, normal_color, selection_color, x0, y0, selected):
...
def xy_menu(
        menu_item,  # it's a list of menu item strs
    ...
    while True:
        ...
        if key == "enter":
            ...
            selected -= 1
            print_menu(menu_item, normal_color, selection_color, x0, y0,
                       selected)
...
def select_menu(
    ...
    for i in range(len(menu_item)):
        if i == selected:
            print(fgcolor.reset + " " * front_space + selection_color +
                  menu_item[i])
        else:
    ...
    while True:
        key = direction_and_enter()
        if key == "enter":
            print(fgcolor.reset + " " * front_space +
                  cursor.down * (len_menu - selected))
            os.system('setterm -cursor on')
            return selected
        elif key == "up" and selected > 0:
            selected -= 1
            print(cursor.up * 2 + cursor.very_left + fgcolor.reset +
                  " " * front_space + selection_color + menu_item[selected])
            print(fgcolor.reset + " " * front_space + normal_color +
                  menu_item[selected + 1] + cursor.up)
        elif key == "down" and selected < (len_menu - 1):
            selected += 1
            print(cursor.up + cursor.very_left + fgcolor.reset +
                  " " * front_space + normal_color + menu_item[selected - 1])
            print(fgcolor.reset + " " * front_space + selection_color +
                  menu_item[selected])
...
def main():
    print(
        select_menu(
            ["1. test", "2. test", "3. test", "4.quit"],
            normal_color=bgcolor.yellow + fgcolor.blue,
            selection_color=format.bold + bgcolor.red + fgcolor.black))
    # while True:
    for _ in range(10):
        char = getch()
        print(ord(char))
