...
def training_room(user):
    print(term_io.delete.whole_screen + term_io.bgcolor.yellow + term_io.fgcolor.black +
          "               The Ancient Tower               " + term_io.fgcolor.reset)
    print("You entered an ancient tower. You will meet spirit fighters with their emojis in the tower. You need beat them to move the every next level. You will receive a blessing at each five levels you clear. But be careful that if you lose a battle, you will lose your emojis in the team. So choose the team wisely.")
    level = 1
    while True:
        if level != 1:
            if (level - 1) % 5 == 0:
                ...
                print(term_io.fgcolor.green +
                      "You gained 10 points." + term_io.fgcolor.reset)
                user.points += 10
            user.save()
        trainer_name = name_gen()
        trainer_team = [randint(1, 6), randint(1, 6), randint(1, 6)]
        while True:
            print(term_io.bgcolor.blue + "\nYou come to the level %d of the tower." %
                  level + term_io.bgcolor.reset)
